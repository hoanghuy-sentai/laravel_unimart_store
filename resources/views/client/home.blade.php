        ...
        <div class="main-content fl-right">
            <div class="section" id="slider-wp">
                <div class="section-detail">
                    ...
                    @if ($item['slideStatus']=="Công khai")
                    <div class="item">
                        {{-- slidePic --}}
                        <img src="{{$item['slidePic']}}" alt="{{$item['slideTitle']}}">
                    </div>
                    @endif
                    @endforeach
                    {{-- <div class="item">
            ...
            <div class="section" id="feature-product-wp">
                ...
                <div class="section-detail">
                    ...
                    <ul class="list-item">
                        ...
                        @if ($item['productFeature']=="Có"&&$item['productStatus']=="Công khai")
                        <li>
                            {{-- asset('client/public/images/img-pro-05.png') |{{route('client.product.test',['id'=>$item->id,'slug'=>Str::slug($item['productName'])])}}--}}
                            ...
                            <a href="{{route('client.product.detail_product',['id'=>$item->id,'slug'=>Str::slug($item['productName'])])}}" title=""
                                class="product-name">{{$item['productName']}}</a>{{-- Laptop Lenovo IdeaPad 120S --}}
                            <div class="price">
                                <span class="new">{{number_format($item['productPrice'],0,',',',')}}đ</span>{{--
                                5.190.000đ --}}
                                <span class="old"></span>{{-- 6.190.000đ --}}
                            </div>
                        </li>
                        @endif
                        @endforeach
            ...
            <div class="section" id="list-product-wp">
                <div class="section-head">
                    <a href="{{route('client.product.product_cat',31)}}"><h3 class="section-title">Điện thoại</h3></a>
                </div>
                <div class="section-detail">
                    <ul class="list-item clearfix">
                        @php
                        ...
                        @endphp
                        @foreach ($products as $item)
                        @if ($item->productcat->parent_id==31)
                        @if ($i<4) <li>
                            <a href="{{route('client.product.detail_product',['id'=>$item->id,'slug'=>Str::slug($item['productName'])])}}" title="" class="thumb">
                                <img src="{{asset($item->productThumb)}}">
                            </a>
            ...
            <div class="section" id="list-product-wp">
                <div class="section-head">
                    <a href="{{route('client.product.product_cat',41)}}"><h3 class="section-title">Laptop</h3></a>
                </div>
                <div class="section-detail">
                    <ul class="list-item clearfix">
                        @foreach ($products as $item)
                        @if ($item->productcat->parent_id==41)
                        <li>
                            <a href="{{route('client.product.detail_product',['id'=>$item->id,'slug'=>Str::slug($item['productName'])])}}" title="" class="thumb">
                                <img src="{{asset($item->productThumb)}}">
                            </a>
                            <a href="{{route('client.product.detail_product',['id'=>$item->id,'slug'=>Str::slug($item['productName'])])}}" title=""
                                class="product-name">{{Str::length($item->productName)>10?Str::substr($item->productName,0,20)."...":$item->productName}}</a>
                            <div class="price">
                                <span class="new">{{number_format($item['productPrice'],0,',',',')}}đ</span>
                                <span
                                    class="old">{{number_format($item['productOldPrice'],0,',',',').'đ'?$item['productOldPrice']>0:""}}</span>
                            </div>
                        </li>
                        @endif
                        @endforeach
